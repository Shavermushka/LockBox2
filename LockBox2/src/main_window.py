...
Добавлено меню «Файл» для создания/открытия баз.
"""
...
class MainWindow(QMainWindow):
    ...
    new_db_requested = pyqtSignal()
    open_db_requested = pyqtSignal()
    ...
    def setup_ui(self):
        ...
        new_db_action.triggered.connect(self.new_db_requested.emit)
        file_menu.addAction(new_db_action)
        open_db_action = QAction("📂 Открыть базу...", self)
        open_db_action.triggered.connect(self.open_db_requested.emit)
    ...
    def apply_theme(self):
        ...
        if os.path.exists(style_path):
            ...
            self.setStyleSheet("""
                QMainWindow { background: #1e1e2a; color: #e8e8f0; }
                QTableWidget { background: #252533; color: #e8e8f0; gridline-color: #3a3a4a; }
                QHeaderView::section { background: #1e1e2a; color: #a0a0b8; }
                QLineEdit, QPushButton { background: #2a2a38; color: #e8e8f0; border: 1.5px solid #3a3a4a; }
            """)
